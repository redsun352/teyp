...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        // Manual TX test panel. It sends only what the user explicitly enters.
        LinearLayout root=findViewById(android.R.id.content).findViewById(R.id.root);
        if(root!=null){
            LinearLayout txRow=new LinearLayout(this);
            txRow.setOrientation(LinearLayout.HORIZONTAL);
            hexInput=new EditText(this);
            hexInput.setHint("HEX: AA 01 00");
            hexInput.setSingleLine(true);
            txRow.addView(hexInput,new LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1));
            txButton=new Button(this);
            txButton.setText("TX AE01");
            txButton.setEnabled(false);
            txButton.setOnClickListener(v->sendManual());
            txRow.addView(txButton,new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,LinearLayout.LayoutParams.WRAP_CONTENT));
            root.addView(txRow,1);
        }
    ...
    private void startScan(){
        ...
        txChar=null;rxChar=null;if(txButton!=null)txButton.setEnabled(false);
    ...
    private void connect(BluetoothDevice d){
        ...
        status.setText("KK-1000 bağlanıyor...");
    ...
    private final BluetoothGattCallback gattCallback=new BluetoothGattCallback(){
        @Override public void onConnectionStateChange(BluetoothGatt g,int st,int ns){
            p("GATT STATE: "+ns+" status="+st);
            if(ns==BluetoothProfile.STATE_CONNECTED){
                gatt=g;status.setText("Bağlandı; servisler aranıyor");p(">>> discoverServices()");g.discoverServices();
            }else if(ns==BluetoothProfile.STATE_DISCONNECTED){
                status.setText("Bağlantı kesildi");
        ...
        @Override public void onServicesDiscovered(BluetoothGatt g,int st){
            ...
            if(svc!=null){
                rxChar=svc.getCharacteristic(AE02);
                txChar=svc.getCharacteristic(AE01);
                if(rxChar!=null){p("AE02 NOTIFY: enabling...");enableNotify(g,rxChar);}
                if(txChar!=null){p("AE01 TX: WRITE_NO_RESPONSE ready");if(txButton!=null)txButton.setEnabled(true);}
            }
        }

        @Override public void onDescriptorWrite(BluetoothGatt g,BluetoothGattDescriptor d,int st){
            p("CCCD WRITE status="+st);
            if(st==BluetoothGatt.GATT_SUCCESS)p("AE02 NOTIFY ACTIVE - waiting for RX.");
        }

        @Override public void onCharacteristicChanged(BluetoothGatt g,BluetoothGattCharacteristic c){
            byte[] v=c.getValue();
            p("RX "+c.getUuid()+" = "+hex(v)+" | ASCII="+ascii(v));
        }

        @Override public void onCharacteristicWrite(BluetoothGatt g,BluetoothGattCharacteristic c,int st){
            p("TX DONE "+c.getUuid()+" status="+st);
        }
    };
    ...
    private void enableNotify(BluetoothGatt g,BluetoothGattCharacteristic c){
        ...
        if(d==null){p("AE02 CCCD bulunamadı");return;}
    ...
    private void sendManual(){
        ...
        if(text.isEmpty()){p("TX: HEX girin");return;}
        try{
    ...
    private byte[] parseHex(String s){
        s=s.replace(","," ").replace("0x","").replace("0X","").trim();
        if(s.isEmpty())return new byte[0];
        String[] parts=s.split("\\s+");
        byte[] out=new byte[parts.length];
        for(int i=0;i<parts.length;i++){
            if(parts[i].length()>2)throw new IllegalArgumentException("Geçersiz byte: "+parts[i]);
    ...
    private String props(int p){
        String z="";
        if((p&BluetoothGattCharacteristic.PROPERTY_READ)!=0)z+=" READ";
        if((p&BluetoothGattCharacteristic.PROPERTY_WRITE)!=0)z+=" WRITE";
        if((p&BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE)!=0)z+=" WRITE_NR";
        if((p&BluetoothGattCharacteristic.PROPERTY_NOTIFY)!=0)z+=" NOTIFY";
        if((p&BluetoothGattCharacteristic.PROPERTY_INDICATE)!=0)z+=" INDICATE";
        return z;
    }
    ...
    private String ascii(byte[] b){
        return new String(b,StandardCharsets.UTF_8).replaceAll("[^\\x20-\\x7E]",".");
    }
