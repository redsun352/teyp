...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        // Manual TX controls. The panel is created programmatically so it does not
        // depend on an R.id.root entry in the XML layout.
        LinearLayout parent=(LinearLayout)log.getParent();
        if(parent!=null){
    ...
    private final BluetoothGattCallback gattCallback=new BluetoothGattCallback(){
        @Override public void onConnectionStateChange(BluetoothGatt g,int st,int ns){
            p("GATT STATE: "+ns+" status="+st);if(ns==BluetoothProfile.STATE_CONNECTED){gatt=g;status.setText("Bağlandı; servisler aranıyor");p(">>> discoverServices()");g.discoverServices();}else if(ns==BluetoothProfile.STATE_DISCONNECTED){status.setText("Bağlantı kesildi");if(txButton!=null)txButton.setEnabled(false);}}
        @Override public void onServicesDiscovered(BluetoothGatt g,int st){
            ...
            BluetoothGattService svc=g.getService(AE00);if(svc!=null){rxChar=svc.getCharacteristic(AE02);txChar=svc.getCharacteristic(AE01);if(rxChar!=null){p("AE02 NOTIFY: enabling...");enableNotify(g,rxChar);}if(txChar!=null){p("AE01 TX: WRITE_NO_RESPONSE ready");if(txButton!=null)txButton.setEnabled(true);}}
        }
    ...
    private void sendManual(){
        ...
        try{byte[] data=parseHex(text);txChar.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);txChar.setValue(data);p("TX AE01 = "+hex(data));p("TX REQUEST="+gatt.writeCharacteristic(txChar));}
