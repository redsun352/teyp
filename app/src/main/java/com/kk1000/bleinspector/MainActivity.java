...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        log=findViewById(R.id.log);status=findViewById(R.id.status);hexInput=findViewById(R.id.hex_input);txButton=findViewById(R.id.tx_button);
        Button btn=findViewById(R.id.scan);btn.setText("SCAN + CONNECT + AE02 NOTIFY");btn.setOnClickListener(v->startScan());txButton.setOnClickListener(v->sendManual());
        BluetoothManager bm=(BluetoothManager)getSystemService(BLUETOOTH_SERVICE);adapter=bm.getAdapter();
        if(Build.VERSION.SDK_INT>=31&&checkSelfPermission(Manifest.permission.BLUETOOTH_SCAN)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.BLUETOOTH_SCAN,Manifest.permission.BLUETOOTH_CONNECT},10);
    }
    private void startScan(){
        if(adapter==null||!adapter.isEnabled()){status.setText("Bluetooth'u aç.");return;}
        if(gatt!=null){try{gatt.close();}catch(Exception ignored){}gatt=null;} txChar=null;rxChar=null;txButton.setEnabled(false);
        out.setLength(0);p("=== KK-1000 BLE INSPECTOR ===");p("15 saniye taranıyor...");scanner=adapter.getBluetoothLeScanner();if(scanner==null){p("BLE scanner kullanılamıyor.");return;}
        scanRunning=true;scanner.startScan(scanCallback);handler.postDelayed(()->{if(scanRunning){scanRunning=false;try{scanner.stopScan(scanCallback);}catch(Exception ignored){}p("=== SCAN BİTTİ ===");}},15000);
    }
    ...
    private final BluetoothGattCallback gattCallback=new BluetoothGattCallback(){
        ...
        @Override public void onServicesDiscovered(BluetoothGatt g,int st){
            p("=== GATT SERVICES ===");for(BluetoothGattService svc:g.getServices()){p("SERVICE: "+svc.getUuid());for(BluetoothGattCharacteristic c:svc.getCharacteristics())p("  CHAR: "+c.getUuid()+" props="+props(c.getProperties()));}
    ...
    private void enableNotify(BluetoothGatt g,BluetoothGattCharacteristic c){if(!g.setCharacteristicNotification(c,true)){p("AE02 notification enable failed");return;}BluetoothGattDescriptor d=c.getDescriptor(CCCD);if(d==null){p("AE02 CCCD bulunamadı");return;}d.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);g.writeDescriptor(d);}
    private void sendManual(){if(gatt==null||txChar==null){p("TX: AE01 bağlı değil");return;}String text=hexInput.getText().toString().trim();if(text.isEmpty()){p("TX: HEX girin");return;}try{byte[] data=parseHex(text);if(data.length==0){p("TX: boş paket");return;}txChar.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);txChar.setValue(data);p("TX AE01 = "+hex(data));p("TX REQUEST="+gatt.writeCharacteristic(txChar));}catch(Exception e){p("HEX ERROR: "+e.getMessage());}}
    private byte[] parseHex(String s){s=s.replace(","," ").replace("0x","").replace("0X","").trim();if(s.isEmpty())return new byte[0];String[] parts=s.split("\\s+");byte[] data=new byte[parts.length];for(int i=0;i<parts.length;i++){if(parts[i].length()>2)throw new IllegalArgumentException("Geçersiz byte: "+parts[i]);data[i]=(byte)Integer.parseInt(parts[i],16);}return data;}
